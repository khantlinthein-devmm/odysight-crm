...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	dtos := make([]ServiceRecordDTO, 0, len(items))
	for _, b := range items {
		dtos = append(dtos, toDTO(b))
	}
	response.JSON(w, http.StatusOK, pagination.Page[ServiceRecordDTO]{Data: dtos, Total: total, Limit: params.Limit, Offset: params.Offset})
}
